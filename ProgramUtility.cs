...
namespace namespaceUtility
{
    class Parse
    {
        public static Graph getGraphByFile(string path, bool directed)
        {
            ...
            while ((lineStr = sr.ReadLine()) != null)
            {
                ...
                else
                {
                    ...
                    createNodeAndEdge(int.Parse(word[0]), int.Parse(word[1]), float.Parse(word[2]), nodeList, edgeList);

                    if (!directed)
                    {
                        createNodeAndEdge(int.Parse(word[1]), int.Parse(word[0]), float.Parse(word[2]), nodeList, edgeList);
                    }
                }
        ...
        static void createNodeAndEdge(int vaterIndex, int sonIndex, float weight, List<Node> nodeList, List<Edge> edgeList)
        {
